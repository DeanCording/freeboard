PluginEditor = function(jsEditor, valueEditor)
{
	...
	function _appendSettingRow(valueCell, newSettings, settingDef,  typeaheadSource, typeaheadDataSegment, currentValue, includeRemove)
	{
		var input;
        ...
        if(settingDef.type == "calculated") {
            ...
            input = $('<input class="calculated-value-input" type="text"/>');
        }
		...
		if(!_.isUndefined(settingDef.suffix)) {
			datasourceToolbox.append($('<li class="input-suffix">' + settingDef.suffix + '</li>'));
		}
        ...
        if(includeRemove) {
            var removeButton = $('<li class="remove-setting-row"><i class="icon-minus icon-white"></i><label></label></li>')
                ...
                datasourceToolbox.append(removeButton);
        }

        wrapperDiv.append(datasourceToolbox);
		...
		if(settingDef.multi_input) {
			input.change(function() {
				var arrayInput = [];
				$(valueCell).find('.calculated-value-input').each(function() {
					var thisVal;
                    if((settingDef.type == "number")||(settingDef.type == "integer")) {
					...
					default:
                    {
                        ...
                        if(settingDef.multi_input) {
                            ...
                            var inputAdder = $('<ul class="board-toolbar"><li class="add-setting-row"><i class="icon-plus icon-white"></i><label>ADD</label></li></ul>')
                                .mousedown(function(e) {
                                    e.preventDefault();
                                    _appendSettingRow(cell, newSettings, settingDef,typeaheadSource, typeaheadDataSegment, null, true);
                                });
                            $(valueCell).siblings('.form-label').append(inputAdder);

                            cell.appendTo(valueCell);
                        }
						...
						if(settingDef.name in currentSettingsValues) {
                            ...
                            if(settingDef.multi_input && _.isArray(currentValue)) {
                                ...
                                for(var i=0; i<currentValue.length; i++) {
                                    _appendSettingRow(cell, newSettings, settingDef, typeaheadSource, typeaheadDataSegment, currentValue[i], includeRemove);
                                    includeRemove = true;
                                }
                            } else {
                                _appendSettingRow(cell, newSettings, settingDef,typeaheadSource, typeaheadDataSegment, currentValue, false);
                            }
                        } else {
                            _appendSettingRow(cell, newSettings, settingDef, typeaheadSource, typeaheadDataSegment, null, false);
                        }
	...
	return {
		...
		{
			createPluginEditor(title, pluginTypes, currentTypeName, currentSettingsValues, settingsSavedCallback);
		}
	}
}
