...
{
    /* Freeboard measures sizes in blocks. A block in freeboard is defined as a rectangle that
     * is fixed at 300 pixels wide and around 45 pixels high. */
	...
	function repositionGrid(repositionFunction)
	{
		...
		rootElement.find("> li").off().removeData();
...
/*				resize: {
	...
	function addPane(element, viewModel, isEditing)
	{
		...
		var height = Number(viewModel.getCalculatedHeight(ROW_HEIGHT+PANE_MARGIN));
	...
	function updatePane(element, viewModel)
	{
		...
		var calculatedHeight = viewModel.getCalculatedHeight(ROW_HEIGHT+PANE_MARGIN);
