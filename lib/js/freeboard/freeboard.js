...
// │ F R E E B O A R D 2                                                │ \\
// ├────────────────────────────────────────────────────────────────────┤ \\
// │ Copyright © 2017 Dean Cording (http://github.com/DeanCording)      │ \\
...
(function($)
{
	function isDOMAttrModifiedSupported(){
		...
		if(p.addEventListener){
			...
			p.attachEvent('onDOMAttrModified', function(){flag = true});
		} else {
			return false;
		}
...
var freeboard = (function()
{
    ...
    var settings = new Settings(theFreeboardModel);
				...
				if(options.operation == 'delete'){
					...
					if(options.type == 'datasource'){
						...
						if(options.operation == 'add'){
							...
							settings.instance_name = viewModel.name();
						}
					}
					else if(options.type == 'widget'){
					...
					pluginEditor.createPluginEditor(title, types, instanceType, settings, function(newSettings){
						...
						if(options.operation == 'add'){
							...
							if(options.type == 'datasource'){
								theFreeboardModel.createDatasource(newSettings);
							}
							else if(options.type == 'widget'){
								viewModel.createWidget(newSettings);
							}
						}
							...
							if(options.type == 'pane'){
								...
								if(options.type == 'datasource'){
									viewModel.name(newSettings.settings.instance_name);
//									delete newSettings.settings.name;
	...
	ko.bindingHandlers.pane = {
		init  : function(element, valueAccessor, allBindingsAccessor, viewModel, bindingContext){
			...
			freeboardUI.addPane(element, viewModel, 'pane-' + theFreeboardModel.panes.indexOf(viewModel),
                                bindingContext.$root.isEditing());
		},
		update: function(element, valueAccessor, allBindingsAccessor, viewModel, bindingContext){
...
console.log('init widget');
            freeboardUI.addWidget($(element), viewModel);
...
console.log('update widget');
            if(viewModel.shouldRender()){
				...
				viewModel.render($(".widget", element));
			}
 ...
 //DOM Ready
        ...
        function resizeEnd(){freeboardUI.processResize(true);}
	...
	return {
		...
		newDashboard        : function(){theFreeboardModel.loadDashboard({allow_edit: true}); },

		loadDashboard       : function(configuration, callback){
            theFreeboardModel.loadDashboard(configuration, callback);
		},

		serialize           : function(){return theFreeboardModel.serialize();},

		setEditing          : function(editing){theFreeboardModel.setEditing(editing);},

        isEditing           : function(){return theFreeboardModel.isEditing(); },
        ...
        resize : function() {freeboardUI.processResize(true); },
		...
		setAssetRoot        : function(assetRoot){ jsEditor.setAssetRoot(assetRoot);},
		...
		showLoadingIndicator: function(show){freeboardUI.showLoadingIndicator(show);},
		...
		getStyleObject      : function(name){return currentStyle[name]; },

		showSettings : function(){settings.showSettings(); }
