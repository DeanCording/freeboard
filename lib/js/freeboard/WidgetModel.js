		...
		if ((newValue in widgetPlugins) && _.isFunction(widgetPlugins[newValue].newInstance)) {
			...
			function finishLoad() {
				widgetType.newInstance(self.settings(), function (widgetInstance) {
					...
					self._widthUpdate.valueHasMutated();
...
// Allow for widgets to send data back to sources
	...
	this._widthUpdate = ko.observable();
    this.width = ko.computed({
        read:function(){
			self._widthUpdate();
			if (!_.isUndefined(self.widgetInstance) && _.isFunction(self.widgetInstance.getWidth)) {
				return self.widgetInstance.getWidth();
			}
